...
.const		counter			= $020a // Interrupts counter (2 bytes)

// Check Busy is done before sending command / data to LCD 
// RS = 0	Write command to LCD (Instruction Register, IR) or read Busy Flag + Address
...
// ----- @Step 4 Entry Mode Set@ -----
			...
			sta counter
			sta counter + 1
			cli						// IDK if it's set by default; just in case...
...
//			lda number				// number to be converted ("number")...
			lda counter				// or Interrupt counter ("counter")...
			sta value
//			lda number +1
			lda counter + 1
			sta value +1
...
check_busy:
			...
			sta DDR_B
BF_busy:
			lda #(0 | RW)			// RS LO = Select IR (used to send cmds or read BF+AC)); R/!W = HI (Read operation)			
			sta PORT_A
			ora #EN					// Strobe EN
			sta PORT_A
			eor #EN
			...
			beq BF_busy				// Compare A and PORT_B MSb (if equal, ZF is set due to no difference, means LCD is busy, hence jump back)
									// 15-03-2022 AIUI it's possible to BIT PORT_B and BPL/BMI; no need to LDA #%10000000
									// http://forum.6502.org/viewtopic.php?f=2&t=4379&hilit=HD44780#p50115
...
//	remove the commenting out (//) to make a 4K ROM
 			*=$fffa
			.word irq
			.word reset
 			.word nmi